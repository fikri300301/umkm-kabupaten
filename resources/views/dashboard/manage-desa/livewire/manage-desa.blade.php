<div>
    <div>
                ...
                <div class="card-header d-flex flex-column ">
                    <div class="mt-3 d-md-inline-flex">
                        <div class="d-block mb-3">
                            <button type="button" wire:click="addManageDesa" class="btn btn-outline-success">
        ...
        <x-modal name="{{ $formAction }}.Desa" wire:model="{{ $formAction }}">
            <x-input name="nama" label="nama Desa" placeholder="blabak" />
            <x-select name="kecamatan" label="Kecamatan">
                @foreach ($kecamatans as $item)
                    <option @selected(@$kecamtan == $item->id || old('kecamatan') == $item->id) value="{{ $item->id }}">
                        {{ $item->name_kecamatan }}
                    </option>
                @endforeach
            </x-select>
