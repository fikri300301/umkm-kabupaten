<div>
    <div>
                ...
                <div class="card-header d-flex flex-column ">
                    <div class="mt-3 d-md-inline-flex">
                        <div class="d-block mb-3">
                            <button type="button" wire:click="addManageBidang" class="btn btn-outline-success">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                    class="bi bi-plus-lg" viewBox="0 0 16 16">
                                    <path fill-rule="evenodd"
                                        d="M8 2a.5.5 0 0 1 .5.5v5h5a.5.5 0 0 1 0 1h-5v5a.5.5 0 0 1-1 0v-5h-5a.5.5 0 0 1 0-1h5v-5A.5.5 0 0 1 8 2Z" />
                                </svg>
                                <span>Tambah Kategori Bidang UMKM</span>
                            </button>
                        </div>
                        ...
                        <table class="table table-hover rounded-md" id="table1">
                            ...
                            <tbody class="table-hover">
                                @forelse ($data as $item)
                                    <tr>
                                        <td>{{ $item->name_bidang }}</td>
                                        <td>{{ $item->status_bidang }}</td>
        ...
        <x-modal name="{{ $formAction }}.Bidang" wire:model="{{ $formAction }}">
            <x-input name="nama" label="nama kategori bidang" placeholder="pengabdian masyarakat" />
            <x-select name="status" label="status kategori bidang">
                @foreach ($status_bidang as $item)
